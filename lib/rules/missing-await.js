      ...
      CallExpression(node) {
        if (node.callee && node.callee.type === "MemberExpression") {
          ...
          if (asyncMatchers.includes(propertyName)) {
            ...
            // Traverse the tree upwards until reaching the statement limit
            while (current && current.type !== "ExpressionStatement" && current.type !== "BlockStatement") {
              
              // If find an await or a return at a higher level (e.g.: await Promise.all)
              if (current.type === "AwaitExpression" || current.type === "ReturnStatement") {
                isHandled = true;
                break;
              }
              
              // Implicit return in Arrow Functions (e.g.: () => expect(...))
              if (current.type === "ArrowFunctionExpression" && current.body === node) {
                isHandled = true;
                break;
              }
            ...
            // Only report if reached the end of the tree and never saw an await
            if (!isHandled) {
              context.report({
                node: node,
